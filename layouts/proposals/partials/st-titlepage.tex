\defineviewerlayer
  [forprinter]
  [printable=yes,state=stop]

\definelayer
  [logo]
  [
    x=\dimexpr(\backspace - 10pt)\relax, % to assure that the logo lines up properly
    y=\topspace,
  ]

\definelayer[monochrome]

\definemakeup
  [titlepage]
  [
    before=\setups{titlepage:setups},
  ]

\startsetups titlepage:setups
  \setupbackgrounds[page][background={color,logo,foreground,monochrome},backgroundcolor=pentandrabrown]
\stopsetups

\starttext

\starttitlepagemakeup

\setlayer[logo]{\externalfigure[pentandra-logotype-reverse.pdf]}

\startcolor[white]
{\bfd \setupinterlinespace <%= @item.fetch(:title).gsub(/\s/, '\\\\\\\\ ') %>\\ Proposal\par\blank[line]}
\stopcolor

\startviewerlayer[forprinter]

\setlayer[logo]{\externalfigure[pentandra-logotype.pdf]}

\setlayer[monochrome]{\framed[frame=off,width=\paperwidth,height=\paperheight,background={color,logo},backgroundcolor=white]{}}

\setlayer[monochrome]{\framed[frame=off,align=right,top=\vss,bottom=\vss,loffset=\backspace,width=\dimexpr(\backspace+\textwidth),height=\paperheight]{{\bfd \setupinterlinespace <%= @item.fetch(:title).gsub(/\s/, '\\\\\\\\ ') %>\\ Proposal\par\blank[line]}}}

\stopviewerlayer

\stoptitlepagemakeup

\stoptext
