...
{\bfb <%= ndashed(last_modified_date_for(@item), type: :plain) %>}
\stopcolor
...
\setlayer[monochrome]{\framed[frame=off,align=right,top=\vss,bottom=\vss,loffset=\backspace,width=\dimexpr(\backspace+\textwidth),height=\paperheight]{{\bfc \setupinterlinespace <%= @item.fetch(:title) %>\\ Proposal\par\blank[line]}{\bfb <%= ndashed(last_modified_date_for(@item), type: :plain) %>}}}
